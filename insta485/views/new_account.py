...
def handle_account():
    ...
    if operation == "login":
        ...
        operation_edit(connection)
    elif operation == "update_password":
        operation_update(connection)
...
def operation_delete(connection):
    ...
    for img in post_images:
        file_path = os.path.join(
            insta485.app.config["UPLOAD_FOLDER"], img["filename"]
        )
        if os.path.exists(file_path):
            os.remove(file_path)
...
def operation_edit(connection):
    ...
    if fileobj:
        ...
        of_path = insta485.app.config["UPLOAD_FOLDER"] / user["filename"]
...
def operation_update(connection):
    """Update password."""
    ...
    new_password_db_string = "$".join(
        [new_algorithm, new_salt, new_password_hash]
    )
